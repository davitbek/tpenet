...
class SeoService extends BaseService
{
    ...
    protected function fixDataForCreate($data)
    {
        ...
        $routeUri = $data['uri'];
        $seoRoute = SeoRoute::where('name', $routeName)->first();

        if ($routeName && ! $seoRoute) {
        ...
        if ($routeUri && $seoRoute->uri != $routeUri) {
            // TODO make logic for write not missing routes with * case
            SeoUri::create([
